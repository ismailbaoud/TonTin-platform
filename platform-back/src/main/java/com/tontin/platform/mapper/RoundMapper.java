...
public interface RoundMapper {
    ...
    @Mapping(target = "amount", expression = "java(request.amount() != null ? request.amount().doubleValue() : null)")
     ...
     */
    @Mapping(target = "dartId", expression = "java(round.getDart() != null ? round.getDart().getId() : null)")
    @Mapping(target = "dartName", expression = "java(round.getDart() != null ? round.getDart().getName() : null)")
    @Mapping(target = "amount", expression = "java(round.getAmount() != null ? java.math.BigDecimal.valueOf(round.getAmount()) : null)")
    @Mapping(target = "recipientMemberId", expression = "java(round.getRecipient() != null ? round.getRecipient().getId() : null)")
    @Mapping(target = "recipientMemberName", expression = "java(round.getRecipient() != null && round.getRecipient().getUser() != null ? round.getRecipient().getUser().getUserName() : null)")
    @Mapping(target = "recipientMemberEmail", expression = "java(round.getRecipient() != null && round.getRecipient().getUser() != null ? round.getRecipient().getUser().getEmail() : null)")
    ...
    default RoundResponse toDtoWithDart(Round round, Dart dart) {
        ...
        if (round.getRecipient() != null) {
            ...
            if (round.getRecipient().getUser() != null) {
                recipientMemberName = round.getRecipient().getUser().getUserName();
                recipientMemberEmail = round.getRecipient().getUser().getEmail();
            }
        }
